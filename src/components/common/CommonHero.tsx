...
const CommonHero = ({ title, firstSubtitle, secondSubtitle, sectionLeft }: HeroProps) => {
    ...
    <section ref={containerRef} className="container flex flex-col font-poppin relative">
      ...
      <div className="flex flex-col items-center">
        <div className="flex justify-center items-center my-20 py-20">
          <span className="text-[18px] font-medium text-primary/80">{firstSubtitle ? firstSubtitle : "Reach Out"}</span>
          <h1 className="has-text-reveal text-[clamp(64px,15vw,300px)] font-semibold px-[80px]" >
            {title}
          </h1>
          <span className="text-[18px] font-medium text-primary/80">{secondSubtitle ? secondSubtitle : "Say Hello"}</span>
        </div>
      ...
      <div className="hero_bottom text-center flex justify-between inner-container w-full mx-auto">
        ...
        <div className="text-[18px] font-semibold font-poppin flex items-center">{sectionLeft ? sectionLeft : "Imranpers@gmail.com"}</div>
      </div>
    </section>
  )
}
